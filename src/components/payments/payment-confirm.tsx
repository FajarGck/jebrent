...
export default function PaymentConfirm({ payment, isAdmin = false }: PaymentConfirmProps) {
  const router = useRouter();
  const [isPending, startTransition] = useTransition();
  const [activeAction, setActiveAction] = useState<string | null>(null);
  const [error, setError] = useState<string | null>(null);

  async function handleAction(action: string, fn: () => Promise<{ error?: string; success?: boolean }>) {
    setError(null);
  ...
  const showActions = payment.status === 'pending_confirmation' && isAdmin;
    ...
    <div className="space-y-3">
      ...
      {payment.proof_image_url && (
        <div className="space-y-1.5">
          ...
          <a href={payment.proof_image_url} target="_blank" rel="noopener noreferrer" className="group block">
            <div className="relative overflow-hidden rounded-xl border border-border">
              <img src={payment.proof_image_url} alt="Bukti pembayaran" className="max-h-48 w-full object-contain bg-white" />
