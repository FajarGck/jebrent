...
export default async function AdminPaymentsPage({ searchParams }: { searchParams: Promise<{ status?: string }> }) {
  ...
  const filtered = filterStatus && filterStatus !== 'all' ? payments.filter((p) => p.status === filterStatus) : payments;
    ...
    <div className="space-y-6">
      <div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted">{payments.filter((p) => p.status === 'pending_confirmation').length} pembayaran menunggu konfirmasi</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex gap-1 overflow-x-auto rounded-xl border border-border bg-card p-1">
        {tabs.map((tab) => (
          ...
          >
            {tab.label}
            {tab.count > 0 && <span className={`ml-1.5 rounded-full px-1.5 py-0.5 text-[10px] ${activeTab === tab.value ? 'bg-white/20' : 'bg-card-muted'}`}>{tab.count}</span>}
          </Link>
        ))}
      </div>
      ...
      {filtered.length === 0 ? (
        ...
        <div className="space-y-3">
          {filtered.map((payment) => {
            ...
            return (
              <Link key={payment.id} href={`/bookings/${payment.booking_id}`} className="block rounded-2xl border border-border bg-card p-4 transition-all hover:border-primary/30 hover:shadow-md">
                <div className="flex items-center justify-between gap-4">
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      <p className="text-sm font-semibold truncate">{vehicle ? `${vehicle.brand} ${vehicle.model}` : '—'}</p>
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-xs text-muted capitalize">{payment.payment_method.replace('_', ' ')}</p>
                  </div>
                </div>
              </Link>
            );
