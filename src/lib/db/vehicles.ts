...
export async function getVehicles(filters?: VehicleFilters): Promise<VehicleWithImages[]> {
  ...
  let query = (supabase
    .from("vehicles")
    .select("*, vehicle_images(*), profiles(full_name, avatar_url)") as any);
...
export async function getVehicleById(id: string): Promise<VehicleWithImages | null> {
  ...
  const { data, error } = await (supabase
    .from("vehicles")
    .select("*, vehicle_images(*), profiles(full_name, avatar_url)")
    .eq("id", id)
    .single() as any);
...
export async function updateVehicle(
  id: string,
  updates: Partial<Omit<Vehicle, "id" | "created_at">>
...
export async function insertVehicleImage(
  image: Omit<VehicleImage, "id">
): Promise<{ data: VehicleImage | null; error: string | null }> {
  const supabase = await createClient();
  const { data, error } = await (supabase
...
export async function setVehiclePrimaryImage(
  ...
  const supabase = await createClient();
  await (supabase.from("vehicle_images") as any)
    .update({ is_primary: false })
    .eq("vehicle_id", vehicleId);
  const { error } = await (supabase.from("vehicle_images") as any)
